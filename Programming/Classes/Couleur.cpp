...
const Couleur Couleur::ORANGE(204, 102, 45, "Orange");
...
*/
Couleur::Couleur():rouge(0),vert(0),bleu(0),pNom(NULL)
{
...
*/
void Couleur::setRouge(int valeurRouge) throw(InvalidColorException)
{
...
*/
void Couleur::setBleu(int valeurBleu) throw(InvalidColorException)
{
...
*/
void Couleur::setVert(int valeurVert) throw(InvalidColorException)
{
...
Couleur Couleur::operator+(const Couleur& couleur) const
{
	...
	if(strcmp(c.getNom(), "")) {
		if(strcmp(couleur.getNom(), "")) {
			int tailleN = 16 + strlen(c.getNom()) + strlen(couleur.getNom());
			char tmp[tailleN];
			strcpy(tmp, "");
			strcpy(tmp, "Melange de ");
			strcat(tmp, c.getNom());
			strcat(tmp, " et ");
			strcat(tmp, couleur.getNom());
			c.setNom(tmp);
		} else {
			c.setNom("Melange Random");
		}
	}
...
void Couleur::Save(std::ofstream& fichier) const
{
		...
		int tailleNom = strlen(this->getNom());
		fichier.write((char *)&tailleNom, sizeof(int));
		fichier.write((char *)pNom, tailleNom);
...
void Couleur::Load(std::ifstream& fichier)
{
		...
		pNom = new char[tailleNom + 1];
		fichier.read((char *)pNom, tailleNom);
