...
{
	Trace("Constructeur par defaut de la classe Rectangle");
	setDimX(0u);
	setDimY(0u);
	setRempli(false);
}
...
{
	Trace("Dans le constructeur d'initialisation de la classe Rectangle");
	setDimX(dimX);
	setDimY(dimY);
	setRempli(rempli);
	this->setId(pId);
...
void Rectangle::setId(const char *pId)
{
	if(pId != NULL)
	{
		if(strlen(pId) > 6) throw BaseException("[-] Le nb de caractères est supérieur à 4");
		if( !regex_match(pId, regex("[R-r][0-9]{3}")) ) throw BaseException("[-] Format incorrect ! Exemple : R452 (OK) "); 
		else {
			strcpy(this->pId, pId); 
		}
	}
