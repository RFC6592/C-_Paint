...
Bouton::Bouton() : pNom(NULL)
{
	Trace("Dans le constructeur par defaut de la classe Bouton");
	setNom("B_0");
	setP1(Point(0, 0));
	setP2(Point(0, 0));
	setSelected(0);
...
{
	Trace("Dans le constructeur de copie de la classe Bouton");
	setP1(b.getP1());
	setP2(b.getP2());
	setNom(b.getNom());
...
Bouton::Bouton(const Point &p1, const Point &p2): pNom(NULL)
{
	Trace("Dans le constructeur d'initialisation de la classe Bouton");
	setNom("B_0");
	setP1(p1);
	setP2(p2);
	setSelected(0);
...
Bouton::Bouton(const char *pNewNom, const Point &p1, const Point &p2): pNom(NULL)
{
	Trace("Dans le constructeur d'initialisation de la classe Bouton");
	setNom(pNewNom);
	setP1(p1);
...
Bouton::~Bouton()
{
	Trace("Dans le destructeur de la classe Bouton");
//	if(pNom) delete[] pNom;
}
...
{
	Trace("Dans la méthode setNom() de la classe Bouton");
	if(pNom) delete pNom;
