...
//  // Ajout de figures
  ...
  d.ajouteFigure(new Ligne("L1",Point(20,20),Point(170,170),NULL),"Jaune");  
...
//   Dessin dans la fenetre graphique + Save et Load
...
//  d.afficheFigures();
  ...
  cout << "--> Enregistrement sur disque..." << endl;
  d.Save("MonDessin.des");
  cout << "Cliquez pour fermer la fenetre et recharger le dessin..." << endl;
  WindowSDL::waitClick();
  WindowSDL::close();

  cout << "--> Chargement du dessin..." << endl;
  Dessin d2;
  d2.Load("MonDessin.des");

  WindowSDL::open(800,600);
  WindowSDL::setBackground(d2.getFond().getRouge(),d2.getFond().getVert(),d2.getFond().getBleu());
  d2.Dessine();
