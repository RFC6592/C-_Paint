...
Ligne::Ligne(const char *pId, const Point &p1, const Point &p2, const Couleur *pCouleur) : Figure(p1, pCouleur)
{
	setExtremite(p2);
	this->setId(pId);
}
...
void Ligne::Dessine(void) const {
	...
	Trace("\x1b[31m R %d G %d B %d | X %d Y %d\x1b[0m", pCouleur->getRouge(), pCouleur->getVert(), pCouleur->getBleu(), position.getX(), position.getY(), extremite.getX(), extremite.getY() );
...
//void Ligne::Load(std::ifstream& fichier, Liste<Couleur>& palette)
//{
//	extremite.Load(fichier);
//	Figure::Load(fichier, palette);
